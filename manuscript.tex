...
Complex, long duration transfer trajectories are highly sensitive to the initial guess and generally have a small region of convergen.
Computation of the reachable set alleviates the need to generate an accurate initial guess for optimization.
From the reachable set, we chose a trajectory which minimizes a metric towards the desired target.
...
In spite of this, it is still possible to determine periodic solutions through the application of a \Poincare map with surface of section chosen normal to a surface in the phase space~\cite{scheeres2000}.
...
Maximization of \( J \), or the minimization of \( -J \), along various directions on the \Poincare section allows us to generate the largest reachability set under the bounded control input.
...
We use the matrix \( Q = \text{diag} \begin{pmatrix} 1 & 0 & 1 & 1 & 0 &1 \end{pmatrix} \in \R^{6 \times 6}\) to represent the mapping onto \( \Sigma \).
...
Since the \Poincare section is four-dimensiona, we parameterize a direction in \( \R^4 \)  using three angles \( \phi_1, \phi_2 , \phi_3 \).
The terminal constraints are given as
...
Similarly, we also add another Lagrange multipler, \( \beta \in \R^{5 \times 1}\), to account for this addiitonal terminal constraint.
