...
Several asteroids have been accurately mapped from a combination of Earth based sensors as well as visiting spacecraft (CITE \href{http://sbn.psi.edu/pds/resource/rshape.html}{http://sbn.psi.edu/pds/resource/rshape.html}). 
Only the body-fixed vectors, and their associated topology, is required to define the exterior gravitational model.
References~\citenum{werner1994} and~\citenum{werner1997} give a detailed derivation of the polyhedron model from first principles.
Here, we summarize the key developments and equations required for implementation and clarity.

Insert picture of face and vectors
...
Once a position vector \( \vecbf{r} \) is defined then the scalars~\cref{eq:edge_factor,eq:face_factor} can be computed for each face and edge.
...
\Cref{eq:jacobi} divides the phase space into distinct realms of possible motion.
...
Figure of zero velocity surfaces for each of the three planes
