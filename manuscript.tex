...
There has been a large amount of work on the optimal control of spacecraft orbital transfers in the three-body problem~\cite{mingotti2011,grebow2011}.
The dynamics of a spacecraft about small bodies is very similar to that of the three-body problem and much of the theory developed there is also applicable.
...
% Detail of work - more empasis and add the novelty of our work
...
The \Poincare section reduces the dimensionality of teh system dynamics to the study of a related discrete update map.
This allows for the design of complex transfer trajectories through the use of a lower dimensional space.
Rather than relying on intuition or insight into the problem, trajectory states are chosen which minimize a distance metric toward a desired target on the \Poincare section.
...
\begin{align}\label{eq:edge_dyad}
    \vecbf{E}_{12} = \hat{\vecbf{n}}_A \parenth{\hat{\vecbf{n}}_{12}^A}^T + \hat{\vecbf{n}}_B \hat{\vecbf{n}}_{21}^B \in \R^{3 \times 3}.
\end{align}
...
The per-edge factor \( L_e \in \R^{1}\), for the edge connecting vertices \( \vecbf{v}_i \) and \( \vecbf{v}_j \), with a constant length \( e_{ij} \) is
...
In this situation, the state is defined as \( \vecbf{x} = \bracket{\vecbf{r}~\>\vecbf{v}}^T\) with \(\vecbf{r} = \bracket{x~\>y~\>z}^T \in \R^{3\times1}\) and \(\vecbf{v}= \bracket{ \dot{x}~\>\dot{y}~\>\dot{z} }^T \in \R^{3\times1}\) representing the position and velocity with respect to the body-fixed frame, respectively.
